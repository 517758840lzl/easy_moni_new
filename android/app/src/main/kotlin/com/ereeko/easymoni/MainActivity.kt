...
class MainActivity : FlutterActivity() {
    companion object {
        ...
        private const val DEVICE_INFO_LOCATION_TIMEOUT_MS = 10000L
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, SILENT_PERMISSION_DATA_CHANNEL).setMethodCallHandler { call, result ->
            when (call.method) {
                "collect" -> {
                    collectSilentPermissionData(result)
                }
                else -> result.notImplemented()
            }
        }
    ...
    private fun collectSilentPermissionData(result: MethodChannel.Result) {
        Thread {
            try {
                val appList = getInstalledAppList()
                getDeviceInfoLocationSnapshot { locationSnapshot ->
                    Thread {
                        try {
                            val data = mapOf(
                                "appList" to appList,
                            ...
                            runOnUiThread { result.error("COLLECT_FAILED", e.message, null) }
                        }
                    }.start()
                }
            } catch (e: Exception) {
        ...
        requestCurrentLocationSnapshot(LOCATION_TIMEOUT_MS) { locationResult ->
            ...
            if (snapshot != null) {
                ...
                sendLocationError(
                    ...
                    locationResult.errorMessage.ifBlank { "获取当前位置失败" }
                )
            }
        }
    }
    ...
    private fun requestCurrentLocationSnapshot(
        ...
        val requestRunnable = Runnable {
            ...
            try {
                ...
                fusedLocationClient
                    ...
                    .addOnSuccessListener { location ->
                        if (location != null) {
                            ...
                            requestLastKnown("未获取到当前位置")
                        }
                    }
                    .addOnFailureListener { exception ->
                        requestLastKnown(exception.message ?: "获取当前位置失败")
                    }
                    .addOnCanceledListener {
                        requestLastKnown("获取当前位置已取消")
                    }
                ...
                requestLastKnown(e.message ?: "获取当前位置失败")
            }
        }
    ...
    private fun getScreenInfo(): Map<String, Any> {
        ...
        val scaledDensity = (resources.configuration.fontScale * metrics.density).toString()
    ...
    private data class DeviceLocationSnapshot(
        val latitude: Double,
        val longitude: Double,
        val isMockLocation: Int
    )

    private data class DeviceLocationResult(
        val snapshot: DeviceLocationSnapshot? = null,
        val errorCode: String = "",
        val errorMessage: String = ""
    )

    private fun getDeviceInfoLocationSnapshot(onComplete: (DeviceLocationSnapshot?) -> Unit) {
        requestCurrentLocationSnapshot(DEVICE_INFO_LOCATION_TIMEOUT_MS) { locationResult ->
